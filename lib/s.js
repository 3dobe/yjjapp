...
module.exports = function(app, config) {
	...
	app.get('/do/s/filelist',function(req, res){
		var spkid = req.session['speakerId'],
            ...
            list = fileList.slice(index, index + pageSize);
		res.send({
			ok: 1,
	...
	app.get('/do/s/msglist', function(req, res){
		var spkid = req.session['speakerId'],
            ...
            index = pageSize * (page - 1),
            list = msgList.slice(index, index + pageSize);
		res.send({
			ok : 1,
	...
	app.get('/do/s/votelist', function(req, res){
		var spkid = req.session['speakerId'],
            ...
            index = pageSize * (page - 1),
            list = voteList.slice(index, index + pageSize);
		res.send({
			ok : 1,
	...
	app.post('/do/s/addvote',function(req, res){
        ...
        if (words.length > _.uniq(words)) {
            return res.send({
                msg : "重复选项"
            });
        }
		var vote = new Vote(voteId, content, words);
	...
	app.get('/do/s/lotterylist', function(req, res){
		var spkid = req.session['speakerId'],
            ...
            index = pageSize * (page - 1),
            list = lotteryList.slice(index, index + pageSize);
		res.send({
			ok : 1,
            total: lotteryList.length,
            lotteryList : list
		});
	});
	/* 查看抽奖 */
