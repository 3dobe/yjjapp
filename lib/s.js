...
module.exports = function(app, config) {
    ...
    app.get('/do/s/audiencelist', function(req, res){
        ...
        res.send({
            ok : 1, audienceList : audienceList
        })
    });
    ...
    app.post('/do/s/addlottery',function(req, res){
        var spkid = req.session['speakerId'],
            ...
            lottery = new Lottery(lotteryId, content, words);
    ...
    /* 改变抽奖状态 */
    app.get('/do/s/setlottery',function(req, res){
        var spkid = req.session['speakerId'],
            lectid = lectureTaffy({
                spkid: spkid
            }).first().id,
        ...
        if (! _.contains(Lottery.States, state)) {
            return res.send({
                msg: '状态不合法'
            })
        }
        var msg = '';
        if (state === Lottery.States.ACTIVE) {
            lottery.activate();
            msg = '抽奖已开启';
        } else if (state === Lottery.States.COOL) {
            lottery.inactivate();
            msg = '抽奖已关闭';
        }
        res.send({
    ...
    app.get('/do/s/me', function(req, res) {
        ...
        res.send({
            'ok': 1,
            'spkid': spkid || 0,
            'lectid': speaker ? speaker.lectid : 0
        });
    });
}